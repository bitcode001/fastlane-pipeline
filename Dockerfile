...
RUN $ANDROID_HOME/cmdline-tools/bin/sdkmanager --sdk_root=${ANDROID_SDK_ROOT} --update \
    ...
    && rm $HOME/packages.txhttps://sdkman.io/#:~:text=Meet%20SDKMAN!%20%E2%80%93,SDKMAN!
RUN source "${HOME}/.sdkman/bin/sdkman-init.sh" \
    && sdk install gradle ${GRADLE_VERSION}

# Install Flutter
ENV FLUTTER_VERSION=flutter-3.19-candidate.5
ENV FLUTTER_HOME=/opt/flutter
ENV PATH=$PATH:$FLUTTER_HOME/bin

RUN git clone --branch $FLUTTER_VERSION https://github.com/flutter/flutter.git $FLUTTER_HOME \
    && $FLUTTER_HOME/bin/flutter doctor --android-licenses \
